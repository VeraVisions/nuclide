...
ent_for_mod()
{
	...
	mkdir -p "./$1/scripts"
	ENT_OUTFILE="./$1/scripts/entities.def"
	echo '' > "$ENT_OUTFILE"
	...
	find ./$1/ -type f \( -iname \*.def \) | grep "decls/def" | while read EDEF_N; do
		echo "parsing decl $EDEF_N..."
		ed_for_def "$EDEF_N" >> "$ENT_OUTFILE"
	done;

	cat ./Tools/base_entities.def >> $ENT_OUTFILE
}

# first dump all the general purpose entities
BASE_ENT="./Tools/base_entities.def"
echo '' > "$BASE_ENT"
